...
app.get('/musicians', async (req, res, next) => {
  ...
  // Your code here
  //* Took this out to refactor with bandFields
  //   if (bandName) {
  //     query.include.push({
  //       model: Band,
  //       where: { name: bandName },
  ...
  if (musicianFields) {
    if (musicianFields.includes('all')) {
      ...
      query.attributes = ['id']; //returning only the id as when no attributes were returned ran into sequelize error
    } else {
  ...
  /* {
    ...
    */
  //* ran into error where band inclusion existed twice, created a conditional to check for both bandName and bandField to update model incluision only 1x
  if (bandName || bandFields) {
    let bandInclude = {
      model: Band,
      required: true,
      where: {},
    ...
    if (bandName) {
      bandInclude.where.name = bandName;
    }

    if (bandFields) {
      if (bandFields.includes('all')) {
        bandInclude.attributes = { exclude: [] };
      } else if (bandFields.includes('none')) {
        bandInclude.attributes = [];
      } else {
        bandInclude.attributes = bandFields;
      }
    } else {
      bandInclude.attributes = ['id', 'name'];
    }
    query.include.push(bandInclude);
  }
  ...
  if (order && order.length) {
    console.log(order);
    query.order = order.map((o) => {
      let [attribute, direction] = o.split(',');
